 ...
 * @author:kekeair
 * 
 * @time:2017年3月21日 上午9:50:55
 * 
 */
public class OutExcelForMakeBillView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> arg0, HSSFWorkbook workbook, HttpServletRequest req,
		...
		List<GetDemandListByCustomerId> list = demandService.getMakeBillByBIllIdAndTeanId(billId, teamId);
		String typeNm = "出库单";
		...
		for (GetDemandListByCustomerId getDemandListByCustomerId : list) {
			tatolCounts +=Double.parseDouble(getDemandListByCustomerId.getMoney());
		}
		...
		for (int i = 0; i < len; i++) {
			...
			String code = list.get(0).getBillCode();
			...
			String customerNm = list.get(0).getCustomerNm();
			...
			cell41.setCellValue(" " + customerNm);
			...
			if (size < 25) {
				...
				for (int j = 0; j < size; j++) {
					GetDemandListByCustomerId listByCustomerId = list.get(countSize + j);
					HSSFCell cell20 = getCell(sheet, count + j + 5, 0);
					cell20.setCellStyle(style4);
					cell20.setCellValue(listByCustomerId.getOrderIndex());
					HSSFCell cell21 = getCell(sheet, count + j + 5, 1);
					cell21.setCellStyle(style4);
					String code2 = listByCustomerId.getCode();
					if (StringUtils.isEmpty(code2)) {
						String createId = "010107060" + String.valueOf(countSize + j);
				...
				for (int j = 0; j < 25; j++) {
					...
					HSSFCell cell21 = getCell(sheet, count + j + 5, 1);
					cell21.setCellStyle(style4);
					String code2 = listByCustomerId.getGoodsCode();
					if (StringUtils.isEmpty(code2)) {
						String createId = "010107060" + String.valueOf(countSize + j);
						cell21.setCellValue(createId);
					} else {
