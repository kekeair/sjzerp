 ...
 * 
 * @author Mr chen
 * @name : ExportOutDemandBillByStateAndTimeView
 * @todo : [客户申报汇总]
 * 
 *       创建时间 ： 2016年12月20日上午11:16:15
 */
public class ExportOutDemandBillByStateAndTimeView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> arg0, HSSFWorkbook workbook, HttpServletRequest req,
		...
		List<ExcelDemandBillByCondition> list = demandService.getExcelOutDemandListHBill(state, stime, etime,
				user.getStructId(), user.getRoleId(), customerId);
		...
		for (int i = 0; i < list.size(); i++) {
			ExcelDemandBillByCondition excel = list.get(i);
			// 日期
			...
			cell27.setCellValue(excel.getDemandNum());
			// 单价
			...
			cell300.setCellValue(excel.getMonay());
			
			totalMoney += excel.getMonay();
			count=count+1;
		}
