				...
				if (goodsIdStr.length() > 0) {
					...
					if (supplierList != null && supplierList.size() > 0) {
						for (int j = 0; j < supplierList.size(); j++) {
							...
							SupplierSelData jiCaiD = goodsList.get(location);//获取供应商信息
							List<SupplierGoods> list = jiCaiD.getSupplierList();
							...
							for (int i = 0; i < list.size(); i++) {
								if (list.get(i).getSupplierId() == sg.getSupplierId()) {
									...
									if (price == null) {
										jiCaiD.setFrontMoney(NumUtil.trim0(jiCaiD.getFrontPrice() * jiCaiD.getOrderNum()));
										jiCaiD.setFrontPrice(sg.getFrontPrice());
										priceMap.put(sg.getGoodsId(), sg.getFrontPrice());
										jiCaiD.setSupplierId(sg.getSupplierId());
										jiCaiD.setsId(sg.getSupplierId());
										list.set(0, list.get(i));// 将当前的供应商放在第一位置上
										list.set(i, n);// 将第一个元素放在当前位置
									} else {
										price = (price <= sg.getFrontPrice() ? price : sg.getFrontPrice());// 取最小价格
										jiCaiD.setFrontMoney(NumUtil.trim0(price * jiCaiD.getOrderNum()));
										jiCaiD.setFrontPrice(price);
