...
public class ExportTuikuBillListView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> arg0, HSSFWorkbook workbook, HttpServletRequest req,HttpServletResponse response) throws Exception {
		...
		// =============列名称end===============================
		List<ExportTuikuModel> list=tuikuService.exportTuikuList(state,stime,etime,
				customerId,user.getStructId());
		...
		int count=2;
		for (int i = 0; i < l; i++) {
			ExportTuikuModel model=list.get(i);
			...
			cell22.setCellValue(model.getCustomerNm());
			//物料编码
