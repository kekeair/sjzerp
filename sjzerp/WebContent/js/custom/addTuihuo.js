	...
	$.post("../common/getAllCenterGoods.do",{
		...
		if (data.code == 1000) {
			...
			if(billId>0){
				...
				supplierId=$("#supplierSel").val();
				supplierNm=$("#supplierSel").find("option:selected").text();  
			}
...
function getTuihuoD() {
	$.post("getTuihuoGoodsByHeadAtNo.do",{
		billId:billId
	},function(data) {
			if (data.code != 1000) {
				tips(data.msg);
				...
				if(data.data.list!=null&&data.data.list.length>0){
					...
					supplierNm=d.supplierNm;
					$("#supplierSel").val(supplierId);
				}
				...
				$(".goodsNm").each(function(){
					...
					if(index==l-1){
						var tuihuoDId=thisObj.attr("atNo");
						lastEditObj=$("#remark"+tuihuoDId);
						bindTab(lastEditObj);
					}
					index++;
				});	
...
function addRow() {
	...
	var str = '<tr id="newTr'+maxOrder+'" class="newTr" order="'+maxOrder+'">'+
		...
		'<td id="newSupplier'+maxOrder+'" supplierId="'+supplierId+'">'+supplierNm+'</td>'+
...
function delNew(removeOrder){
	...
	$(".newTr").each(function(){
		...
		if(parseInt(thisOrder)>parseInt(removeOrder)){
			...
			$("#newNum"+preOrder).eq(0).attr("id","newNum"+thisOrder);
...
function save(){
	...
	$.post("saveTuihuoD.do?", {
		...
		tips(data.msg);
		setTimeout(function(){
			sessionStorage.tuihuoBillId=data.data;
			location.reload();
		},1000);
	}, "json");	
}
