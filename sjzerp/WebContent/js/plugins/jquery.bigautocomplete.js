(function($) {
	var bigAutocomplete = new function() {
		...
		this.functionalKeyArray = [ 9, 20, 13, 16, 17, 18, 91, 92, 93, 45, 36,
				33, 34, 35, 37, 39, 112, 113, 114, 115, 116, 117, 118, 119,
				120, 121, 122, 123, 144, 19, 145, 40, 38, 27 ];// 键盘上功能键键值数组
		...
		this.init = function() {
					...
					.bind(
							...
							function(event) {
								...
								if ((!($target.parents()
										...
										&& (!$target
												.is(bigAutocomplete.currentInputText))) {
									bigAutocomplete.hideAutocomplete();
								}
							})

			// 鼠标悬停时选中当前行
			$("#bigAutocompleteContent").delegate("tr", "mouseover",
					function() {
			...
			$("#bigAutocompleteContent")
					.delegate(
							...
							"click",
							function() {
								bigAutocomplete.currentInputText.val($(this)
										.find("div:last").html());
		...
		this.autocomplete = function(param) {
			...
			$this.keydown(function(event) {
				...
				case 40:// 向下键
					...
					if ($nextSiblingTr.length <= 0) {// 没有选中行时，选中第一行
						$nextSiblingTr = $bigAutocompleteContent
								.find("tr:first");
					} else {
					...
					if ($nextSiblingTr.length > 0) {// 有下一行时（不是最后一行）
						...
						// div滚动到选中的行,jquery-1.6.1 $nextSiblingTr.offset().top
						// 有bug，数值有问题
						$bigAutocompleteContent
								.scrollTop($nextSiblingTr[0].offsetTop
				...
				case 38:// 向上键
					...
					var $previousSiblingTr = $bigAutocompleteContent
							.find(".ct");
					if ($previousSiblingTr.length <= 0) {// 没有选中行时，选中最后一行行
						$previousSiblingTr = $bigAutocompleteContent
								.find("tr:last");
					} else {
					...
					if ($previousSiblingTr.length > 0) {// 有上一行时（不是第一行）
						...
						// div滚动到选中的行,jquery-1.6.1
						// $$previousSiblingTr.offset().top 有bug，数值有问题
						$bigAutocompleteContent
								.scrollTop($previousSiblingTr[0].offsetTop
										- $bigAutocompleteContent.height()
						...
						if (!isFunctionalKey
							...
							if (data != null && $.isArray(data)) {
								...
								for (var i = 0; i < data.length; i++) {
									if (data[i].goodsNm.indexOf(keyword_) > -1||data[i].goodsCode.indexOf(keyword_)>-1) {
										data_.push(data[i]);
									}
								}
						...
						if (k == 13) {
							...
							if ($bigAutocompleteContent.css("display") != "none") {
								if (callback_ && $.isFunction(callback_)) {
									callback_($bigAutocompleteContent.find(
											".ct").data("jsonData"));
								}
			...
			function makeContAndShow(data_) {
				...
				for (var i = 0; i < data_.length; i++) {
					cont += "<tr><td><div>" + data_[i].goodsNm
							+ "</div></td></tr>"
				}
				cont += "</tbody></table>";
...
function kindSel(){
	...
	$("#kindDiv").css({left:kindOffset.left + "px", top:kindOffset.top 
		+ kindObj.outerHeight()-height + "px"}).slideDown("fast");
...
var selInput;
var modalGoodsList=new Array();//弹出框物料列表
getGoodsKind();
...
$(document).on("click", ".goodsBtn", function(){
	...
	if(rowType=='old'){
		...
		selInput=$("#goodsNm"+atNo);
	}else{
		var order=$(this).attr("order");
		selInput=$("#newGoodsNm"+order);
	}
	$("#goodsModal").modal();
});
$(document).on("dblclick", ".goodstr", function(){
	var goodsId=$(this).attr("goodsId");
	var l=modalGoodsList.length;
	for (var i = 0; i < l; i++) {
		if(modalGoodsList[i].goodsId==goodsId){
			setValue(modalGoodsList[i],selInput);
			break;
		}
	}
...
function getGoodsKind(){
	$.post("../common/getGoodsKind.do?", {
		...
		if(data.code==1000){
			...
			nodesTmpl.unshift(allKind);
			zTree=$.fn.zTree.init($("#kindTree"), setting, nodes);
			zTreeTmpl=$.fn.zTree.init($("#kindTreeTmpl"), settingTmpl, nodesTmpl);
		}
	}, "json");
}
...
//搜索
function search(){
	var kindCode=$("#kindSel").attr("code");
	modalGoodsList.length=0;
	if(kindCode==-1){
		pushModalGoodsList();
	}else{
		var l=goodsList.length;
		for (var i = 0; i < l; i++) {
			if (goodsList[i].goodsCode.startWith(kindCode)){
				modalGoodsList.push(goodsList[i]);
			}
		}
	}
	$("#goodsbody").empty();
